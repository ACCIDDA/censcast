test_that("spec_los returns a survival vector of correct length", {
  s <- spec_los("negbin", mu = 2, k = 1.7, max_stay = 10)
  expect_length(s, 11L)
...
test_that("each family produces a non-increasing survival in [0, 1]", {
  for (fam in list(
    ...
    s <- do.call(spec_los, c(family = fam[[1]], fam[[2]], list(max_stay = 20)))
    expect_length(s, 21L)
    expect_true(all(s >= 0 & s <= 1))
    expect_true(all(diff(s) <= 1e-9))
  }
